...
function loadChatView() {
  ...
  favoritesRef.on("child_removed" , function(snapshot) {
    //キャッシュからお気に入りデータを削除する
    delete dbdata.favorites[snapshot.key];
    //お気に入りページかどうかを判断し、お気に入り一覧ページからメッセージを削除する処理を書く
    if( currentRoomName === "favorites" ){
      $("#message-id-"+snapshot.key).remove();
    }
  });
  ...
  favoritesRef.on("child_added" , function(snapshot) {
    //キャッシュにお気に入りデータを追加する
    console.log("お気に入りデータ",snapshot.key,snapshot.val())
    dbdata.favorites[snapshot.key] = snapshot.val().message;
  });
...
function createMessageDiv(messageId, message) {
  ...
  //お気に入りデータ自体の存在確認
  if( dbdata.favorites ){
    //該当のIDに対するお気に入りデータの存在確認
    if( dbdata.favorites[messageId] ){
      //存在するならば星の初期表示を変える
      $div.find(".glyphicon").removeClass("glyphicon-star-empty").addClass("glyphicon-star");
    }
  }

  // お気に入りボタン表示切替
  $div.find(".glyphicon").click(function() {
    // 1. 白星の場合、黒星にする
...
function _showRoom(roomName) {
  if( roomName === "favorites"){
    console.log("お気に入りルームです:",roomName);
    //グローバル変数に現在の部屋名を保持
    currentRoomName = "favorites";
    
    //入力欄を無効にする
    $(".comment-form__text").attr("disabled", "disabled");
    //送信ボタンも無効にする
    $(".comment-form__submit").attr("disabled", "disabled");
    
    //お気に入りルーム名を変更
    $(".room-list-menu").text("ルーム: お気に入り一覧");
    
    //ドロップダウンメニューのお気に入りをハイライトする
    $(".room-list > li").removeClass("active");
    $("[href='#favorites']").closest("li").addClass("active");
    ...
    if( dbdata.favorites ){
      //お気に入りデータを表示
      $.each(dbdata.favorites,function(messageId,message){
  ...
  //お気に入りページ以外では入力欄を有効にする
  $(".comment-form__text").removeAttr("disabled");
...
//    console.log("Hello");
    ...
    //$(".glyphicon glyphicon-star-empty").off("click");
    // return false; */
  // });
// 
